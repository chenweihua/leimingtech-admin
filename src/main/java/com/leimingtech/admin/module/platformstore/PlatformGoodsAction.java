...
/**
 * action描述:平台关于商品跳转action 创建人：cgl 创建时间：2015年08月03日16:04:35 平台自营
 */
@Controller
...
public class PlatformGoodsAction extends BaseController {
	...
	Map<String, String> findChildClass(@RequestParam(value = "id") String id,
		...
		List<GoodsClass> classList = goodsClassService.findList(Integer
				.parseInt(id));
		String json = "null";
	...
	public String sellDetail(@RequestParam(value = "gcId") String gcId,
		...
		try {
			...
			// 商品分类
			GoodsClass goodsClass = goodsClassService.findById(Integer
					.valueOf(gcId));
			// 类型id
			...
			/*
			 * 通过类型id获得类型下的品牌,规格,属性 首先通过类型id获得goodsTypeVo
			 * 在这个超类中,有3个list,是品牌,规格,属性
			 */
			GoodsTypeVO goodsTypeVO = goodsTypeService
					.selectTypeFetchOther(typeId);
			...
			if (goodsTypeVO != null) {
				...
				if (goodsTypeVO.getAttributes() != null) {
					// 获得该类型下的属性
					List<GoodsAttribute> goodsAttributes = goodsTypeVO
							.getAttributes();
					// 放入model
			...
			Transport transport = transportService
					.getDefTransportByStoreId(storeId);
	...
	String savePro(
		...
		try {
			...
			/** 生成静态页面 */
			ToStaticSendToFront.onegoodsDetailStatic(goodsId,
					goods.getStoreId());
			// 将成功的信号传导前台
	...
	public ModelAndView editgoods(
		...
		/*
		 * 通过类型id获得类型下的品牌,规格,属性 首先通过类型id获得goodsTypeVo 在这个超类中,有3个list,是品牌,规格,属性
		 */
		...
		// 得到goodsSpec的list
		List<GoodsSpec> goodsSpecs = goodsSpecService.findListByGoodsId(goods
				.getGoodsId());
		for (int i = 0; i < goodsSpecs.size(); i++) {
			if (goodsSpecs.get(i).getSpecGoodsSpec() != null
					&& !goodsSpecs.get(i).getSpecGoodsSpec().trim().equals("")) {
				goodsSpecs.get(i).setSpecValueIdStr(
						GoodsUtils.getThisGoodsAllSpecValueId(goodsSpecs.get(i)
								.getSpecGoodsSpec()));
			}
		}
		// 放入model
		...
		if (goodsSpecs.size() == 1) {
			model.addObject("goodsstorenum", goodsSpecs.get(0)
					.getSpecGoodsStorage());
		}
		...
		if (goodsSpec != null && !goodsSpec.trim().equals("")) {
			Map<String, List<GoodsSpecVo>> specMap = GoodsUtils
					.goodsSpecStrToMapList(goodsSpec);
		...
		if (goodsColImg != null && !goodsColImg.trim().equals("")) {
			// 得到map
			Map<String, String> colImgMap = GoodsUtils
					.goodsColImgStrToMap(goodsColImg);
	...
	String updatePro(
		...
		try {
			...
			request.setCharacterEncoding("utf-8");
			String goodsSpecJson = request.getParameter("goodsSpecJson");
			Integer goodsShow = Integer.valueOf(request
					.getParameter("goodsShow"));
			// 获得当前店铺id
			Integer storeId = PlatformInfo.PLATFORM_STORE_ID;
			// 获得当前店铺名称
			String storeName = PlatformInfo.PLATFORM_STORE_NAME;
			...
			/** 生成静态页面 */
			ToStaticSendToFront.onegoodsDetailStatic(goods.getGoodsId(),
					goods.getStoreId());
			// 将成功的信号传导前台
			showSuccessJson("商品数据保存成功");
			return json;
	...
	public String storeList(
		...
		// 获取商品审核状态设置值
		Map<String, String> map = settingService
				.findByNameResultMap("goods_isApply");
		...
		try {
			...
			if (!goodsIdsStr.equals("")) {
				...
				for (int i = 0; i < goodsIds.length; i++) {
					...
					/** 生成静态页面 */
					ToStaticSendToFront.onegoodsDetailStatic(
							goods.getGoodsId(), PlatformInfo.PLATFORM_STORE_ID);
				}
			}
			map.put("success", true);
		} catch (Exception e) {
		...
		try {
			...
			if (!goodsIdsStr.equals("")) {
				...
				for (int i = 0; i < goodsIds.length; i++) {
					...
					toStaticService.deleteGoodsDetailStaticPage(Integer
							.parseInt(goodsIds[i]));
				}
			}
			map.put("success", true);
	...
	public ModelAndView updateCombinationIndex(Integer storeId, Integer goodsId) {
		...
		ModelAndView modelAndView = new ModelAndView(
				"/platform/goods/pro-combination");
		...
		/** 查询 */
		List<GoodsCombination> list = goodsCombinationService
				.selectByCondition(goodsCombination);
	...
	public void loadgoodsbystoreid(HttpServletResponse response)
			throws Exception {
		// 平台自营店铺id
		List<GoodsExcel> goodslist = goodsService
				.findGoodListbystoreid2(PlatformInfo.PLATFORM_STORE_ID);
		if (goodslist.size() != 0) {
			// 定义文件的标头
			String[] headers = { "商品ID", "商品名称", "商品分类id", "商品类型,1为全新、2为二手",
					"商品副标题", "商品店铺价格", "商品货号", "商品推荐 是:1 否:0", "商品关键字",
					"商品描述 ", "商品上架1下架0" };
			String excelurl = ExportExcelUtils.export(goodslist,
					CommonConstants.FILE_BASEPATH
							+ Constants.STORE_goodsexcel_URL, headers);
			...
			try {
				bis = new BufferedInputStream(new FileInputStream(
						CommonConstants.FILE_BASEPATH
								+ Constants.STORE_goodsexcel_URL + excelurl));
